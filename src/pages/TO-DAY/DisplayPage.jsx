...
const DisplayPage = () => {
  ...
  const [selectedStatus, setSelectedStatus] = useState({}); // لتخزين الحالة المحددة لكل إدخال
  const [selectedAction, setSelectedAction] = useState(null); // لتخزين الحالة المحددة للأزرار
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // تصفية البيانات بناءً على تاريخ اليوم
        // @ts-ignore
        const matchingData = allData.filter(item => item.date === today);
        console.log("Filtered data:", matchingData);
        ...
        setIsLoading(false); // إيقاف حالة التحميل حتى في حالة حدوث خطأ
      }
    };
  ...
  const handleSelectChange = async (id, event) => {
    const newStatus = event.target.value;
    setSelectedStatus(prevState => {
      const updatedStatus = { ...prevState, [id]: newStatus };
      // تحديث البيانات في Firestore
      updateDoc(doc(db, "userData", id), { status: newStatus });
  ...
  const filteredDisplayData = selectedAction
    ? filteredData.filter(item => selectedStatus[item.id] === selectedAction)
    : filteredData;
  ...
  return (
    <Fragment>
      ...
      {isLoading ? (
        ...
        <div className="all">
          <div className="Data">
            <div className="grob_btn">
              <h2>Data Vaccinations</h2>

              {/* Buttons outside of the card */}
              <div className="buttons-container">
                <button onClick={() => handleButtonClick("completed")}>Done</button>
                <button onClick={() => handleButtonClick("notCompleted")}> Not Done</button>
                <button onClick={() => handleButtonClick("inProgress")}>To Confirm</button>
                <button onClick={() => handleButtonClick(null)}>Show All</button>
              </div>
            </div>

            <div data-aos="zoom-in" className="inputs">
              {filteredDisplayData.length > 0 ? (
                filteredDisplayData.map((data, index) => (
                  <div className="pet_data" key={index}>
                    <p><i>Owner`s Name:</i> <span>{data.name}</span></p>
                    <p><i>Pet Name :</i><span>{data.Pet_Name}</span></p>
                    <p><i>File number :</i> <span>{data.File_number}</span></p>
                    <p><i>Phone Number :</i><span>{data.phone}</span></p>
                    <p><i>Vaccination :</i> <span>{data.Vaccinations}</span></p>
                    ...
                    <div className="select">
                      <select
                        ...
                        onChange={(event) => handleSelectChange(data.id, event)}
                      >
                        <option value="">Select Status</option>
                        <option value="completed">Done</option>
                        <option value="notCompleted">Not Done</option>
                        <option value="inProgress">To Confirm</option>
                      </select>
                    </div >
                  </div>
                ))
              ) : (
                <p>No matching data for the selected criteria.</p>
              )}
            </div>
          </div>
        </div>
